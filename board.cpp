...
void Board::initializeBoard(SDL_Renderer* gameRenderer){
    ...
    // BLACK
    boardState[0][0] = std::make_unique<Rook>(BLACK, ROOK, gameRenderer); boardState[0][0]->setPosition(0,0);
    boardState[0][1] = std::make_unique<Knight>(BLACK, KNIGHT, gameRenderer); boardState[0][1]->setPosition(0,1);
    boardState[0][2] = std::make_unique<Bishop>(BLACK, BISHOP, gameRenderer); boardState[0][2]->setPosition(0,2);
    boardState[0][3] = std::make_unique<Queen>(BLACK, QUEEN, gameRenderer); boardState[0][3]->setPosition(0,3);
    boardState[0][4] = std::make_unique<King>(BLACK, KING, gameRenderer); boardState[0][4]->setPosition(0,4);
    boardState[0][5] = std::make_unique<Bishop>(BLACK, BISHOP, gameRenderer); boardState[0][5]->setPosition(0,5);
    boardState[0][6] = std::make_unique<Knight>(BLACK, KNIGHT, gameRenderer); boardState[0][6]->setPosition(0,6);
    boardState[0][7] = std::make_unique<Rook>(BLACK, ROOK, gameRenderer); boardState[0][7]->setPosition(0,7);

    // WHITE
    boardState[7][0] = std::make_unique<Rook>(WHITE, ROOK, gameRenderer); boardState[7][0]->setPosition(7,0);
    boardState[7][1] = std::make_unique<Knight>(WHITE, KNIGHT, gameRenderer); boardState[7][1]->setPosition(7,1);
    boardState[7][2] = std::make_unique<Bishop>(WHITE, BISHOP, gameRenderer); boardState[7][2]->setPosition(7,2);
    boardState[7][3] = std::make_unique<Queen>(WHITE, QUEEN, gameRenderer); boardState[7][3]->setPosition(7,3);
    boardState[7][4] = std::make_unique<King>(WHITE, KING, gameRenderer); boardState[7][4]->setPosition(7,4);
    boardState[7][5] = std::make_unique<Bishop>(WHITE, BISHOP, gameRenderer); boardState[7][5]->setPosition(7,5);
    boardState[7][6] = std::make_unique<Knight>(WHITE, KNIGHT, gameRenderer); boardState[7][6]->setPosition(7,6);
    boardState[7][7] = std::make_unique<Rook>(WHITE, ROOK, gameRenderer); boardState[7][7]->setPosition(7,7);

    for (int i = 0; i < 8; i++){
        boardState[1][i] = std::make_unique<Pawn>(BLACK, PAWN, gameRenderer); boardState[1][i]->setPosition(1,i);
        boardState[6][i] = std::make_unique<Pawn>(WHITE, PAWN, gameRenderer); boardState[6][i]->setPosition(6,i);
    }
}
...
void Board::movePiece(const Move& move) {
    ...
    // This logic assumes move.capturedPiece correctly identifies the piece to be captured.
    ...
    if (pieceTaken) {
        // The color of the piece that made the move (move.piece) determines which list.
        // If a black piece moved, it captured a white piece.
        if (move.piece && move.piece->getColor() == BLACK) {
            whiteCapturedPieces.push_back(std::move(pieceTaken));
        } 
        // If a white piece moved, it captured a black piece.
        else if (move.piece && move.piece->getColor() == WHITE) {
            blackCapturedPieces.push_back(std::move(pieceTaken));
        }
    }

    // Determine whose turn it was based on the color of the piece that moved.
    // If a BLACK piece moved, it's now WHITE's turn, and we list pieces captured by BLACK.
    if (move.piece && move.piece->getColor() == BLACK) {
    ...
    if (move.castling == true){
        if (move.isKingSide){
            ...
            // Ensure there's a piece at the rook's starting position
            if (boardState[r1][7] != nullptr) {
                // Get a raw pointer to the piece and attempt to cast it to Rook.
                Rook* actualRook = dynamic_cast<Rook*>(boardState[r1][7].get()); 

                if (actualRook) {
                    // Successfully cast to Rook, now update its state
                    actualRook->setHasMoved(true);
                    actualRook->setIsCastlingEligible(false);
                    actualRook->setPosition(r1, rookDestCol);

                    // Move the rook piece on the board
                    boardState[r1][rookDestCol] = std::move(boardState[r1][7]); // boardState[r1][7] becomes nullptr
                } else {
                    // This should not happen if castling move generation is correct and piece at (r1,7) is indeed the rook
                 ...
                 // Handle error: rook is missing
                 return;
            }
        }
        else if (move.isQueenSide) {
            // Similar logic for Queen-side castling:
            // Ensure there's a piece at the rook's starting position
            int rookDestCol = 3;
            if (boardState[r1][0] != nullptr) {
                ...
                if (actualRook) {
                    ...
                    // Move the rook piece on the board
                    boardState[r1][rookDestCol] = std::move(boardState[r1][0]); // boardState[r1][0] becomes nullptr
                } else {
                    // This should not happen if castling move generation is correct and piece at (r1,7) is indeed the rook
        ...
        if (boardState[r2][c2] && boardState[r2][c2]->getType() == KING) {
            ...
            // Unusual error if castling was identified correctly
    ...
    for(int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
            ...
            if (piece && piece->getColor() == color){
                // Pass the flag to the piece's move generation
...
bool Board::isKingInCheck(Color color) const{
    ...
    std::string colorName = (color == 0) ? "White" : "Black";
...
bool Board::checkIfMoveRemovesCheck(const Move& move){
    ...
    // Revert the move
    // 1. Move the piece back to its original square
    boardState[r1][c1] = std::move(boardState[r2][c2]);
    if (boardState[r1][c1]) {
        boardState[r1][c1]->setPosition(r1, c1);
    }
    // 2. Restore the piece that was originally at the destination square
