...
static void createMainMenu(UIManager* uiManager, std::string& pendingScreen, bool& running, const std::string& fontPath) {
    ...
    auto* customBtn = uiManager->addElement<Button>(700, 560, 90, 32,
        ...
        [panel]() {
            panel->setCustomLayout([](UIPanel& p){
                ...
                int x0 = p.rect.x + 20;
                int y0 = p.rect.y + 20;
                int colW = (p.rect.w - 40 - 12) / 2; // padding and gap
                int x = x0;
                int yLeft = y0;
                int yRight = y0;
                bool left = true;
                for (auto* c : p._debugGetChildren()) { // internal accessor below
                    if (!c) continue;
    ...
    // Global callbacks toggle
    bool callbacksOn = UIConfig::areCallbacksEnabled();
    Label* cbState = uiManager->addElement<Label>(170, 530, std::string("Callbacks: ") + (callbacksOn ? "ON" : "OFF"),
        tupleToColor(180,180,180,255), 14, fontPath);
    Button* toggleCallbacks = uiManager->addElement<Button>(290, 526, 180, 28,
        callbacksOn ? "Disable callbacks" : "Enable callbacks",
        [](){},
        tupleToColor(150,150,150,255), tupleToColor(180,180,180,255), fontPath, tupleToColor(0,0,0,255), 4, 14);
    // Ensure the toggle can change the global flag even when callbacks are currently disabled
    toggleCallbacks->setBypassCallbackGate(true);
    // Set its callback now that we have the pointer
    toggleCallbacks->setCallback([cbState, toggleCallbacks]() {
        bool now = !UIConfig::areCallbacksEnabled();
        UIConfig::setCallbacksEnabled(now);
        cbState->setText(std::string("Callbacks: ") + (now ? "ON" : "OFF"));
        toggleCallbacks->setText(now ? "Disable callbacks" : "Enable callbacks");
    });
